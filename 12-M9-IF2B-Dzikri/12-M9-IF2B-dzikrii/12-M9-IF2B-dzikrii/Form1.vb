...
Public Class Form1
    ...
    Sub TampilkanData()
        ...
        conn.Open()
        da = New SQLiteDataAdapter("SELECT * FROM tbl_mhs", conn)
        dt = New DataTable()
        da.Fill(dt)
    ...
    Function DataValidation() As Boolean
        ...
        If String.IsNullOrWhiteSpace(txtNama.Text) Then
            MessageBox.Show("Nama tidak boleh kosong", "Mohon isi terlebih dahulu", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            txtNama.Focus()
            Return False
        End If
        If String.IsNullOrWhiteSpace(txtKelas.Text) Then
            MessageBox.Show("Kelas tidak boleh kosong", "Mohon isi terlebih dahulu", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            txtKelas.Focus()
            Return False
        End If
        Return True
    ...
    Private Sub btnCreate_Click(sender As Object, e As EventArgs) Handles btnCreate.Click
        ...
        conn = New SQLiteConnection(dbPath)
        conn.Open()
        Dim insertSql As String = "INSERT INTO tbl_mhs (NIM, NAMA, KELAS) VALUES (@nim, @nama, @kelas)"
    ...
    Function deleteValidation() As Boolean
        If String.IsNullOrWhiteSpace(txtNIM.Text) Then
            MessageBox.Show("NIM tidak boleh kosong untuk menghapus data!", "Mohon isi terlebih dahulu", MessageBoxButtons.OK, MessageBoxIcon.Warning)
            txtNIM.Focus()
            Return False
        End If
        Return True
    ...
    Private Sub btnDelete_Click(sender As Object, e As EventArgs) Handles btnDelete.Click
        ...
        If MessageBox.Show("Yakin ingin menghapus data?", "Konfirmasi", MessageBoxButtons.YesNo) = DialogResult.Yes Then
            conn = New SQLiteConnection(dbPath)
            conn.Open()
            Dim deleteSql As String = "DELETE FROM tbl_mhs WHERE NIM=@nim"
    ...
    Private Sub btnUpdate_Click(sender As Object, e As EventArgs) Handles btnUpdate.Click
        ...
        conn = New SQLiteConnection(dbPath)
        conn.Open()
        Dim updateSql As String = "UPDATE tbl_mhs SET NAMA=@nama, KELAS=@kelas WHERE NIM=@nim"
    ...
    Private Sub txtCari_TextChanged(sender As Object, e As EventArgs) Handles txtCari.TextChanged
        conn = New SQLiteConnection(dbPath)
        conn.Open()
        Dim cariSql As String = "SELECT * FROM tbl_mhs WHERE NIM LIKE @cari OR NAMA LIKE @cari OR KELAS LIKE @cari"
